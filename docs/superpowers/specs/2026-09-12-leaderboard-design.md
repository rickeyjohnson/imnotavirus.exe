...
INAV.leaderboard
  ...
  me()                      -> { clientId, name }   // name may be null
```

A row is `{ id, rank, name, score, won, mine }`. `cut` is the rank the visible
list stops at, so the UI knows when to pin the player's own row below a
separator instead of scrolling to find it.
...
toggles to force each, so the loading, empty, offline and error states get
designed now rather than discovered in phase two. Phase two replaces that one
file; nothing else in the game knows which source is behind the seam.
