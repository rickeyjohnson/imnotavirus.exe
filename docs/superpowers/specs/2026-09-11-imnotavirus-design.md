...
| Sound toggle (2026-09-13) | A speaker button in the stage's **top-right corner**, above every screen including the crash screen; a red slash when muted. The choice persists in `localStorage` | Pop-ups in the top-right grid cell used to spawn with their close button exactly where the toggle sits, so a player aiming for the ✕ would mute the game instead. Spawns are pushed below the toggle; measured over 900 pop-ups, none overlapped it, at a cost of 75.3% -> 75.1% desktop coverage |
