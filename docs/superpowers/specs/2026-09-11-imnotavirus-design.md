...
### 5.6 Tuning
Every knob lives in `js/config.js` — round length, cap, the interval curve, burst gaps and sizes, pop-up sizes, the spawn grid, ghost behaviour and every delay. Read that file for current values rather than duplicating them here. Balance, measured over 2000 simulated rounds per click speed, each round rolling its own ramp: **3.0 clicks/second never survives, 3.2 wins 6% of the time, 3.5 is a coin flip at 53%, 3.8 wins 97%.** That band sits at the top of what aimed clicking at this target size allows, which is deliberate. A round emits 146-170 pop-ups depending on the roll. An unattended round crashes at about 0:17.
